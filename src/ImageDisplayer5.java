...
public class ImageDisplayer5 extends JPanel{
        ...
        private final static int WIDTH = 876;
        ...
        public void drawImage() {
            try {
                ...
                image = ImageIO.read(new File(fileLocation + imagens.get(Interface.numFotoShowReelTrue)));
                BufferedImage imgInput = ImageIO.read(new File(fileLocation + imagens.get(Interface.numFotoShowReelTrue)));
                if(imgInput.getWidth() > imgInput.getHeight()){
                    int scaledWidth = 876/imgInput.getWidth();
                    if(scaledWidth * imgInput.getHeight() > 750){
                        thumbnail = imgInput.getScaledInstance(-1, 570, Image.SCALE_SMOOTH);
                    }else{
                        thumbnail = imgInput.getScaledInstance(876, -1, Image.SCALE_SMOOTH);
                    }
                }else{
                    thumbnail = imgInput.getScaledInstance(-1, 570, Image.SCALE_SMOOTH);
                }
...
//                image.getGraphics().drawImage(thumbnailFundo, WIDTH/2 - thumbnail.getWidth(this)/2, HEIGHT/2 - thumbnail.getHeight(this)/2, null);
                ...
                image = new BufferedImage(thumbnail.getWidth(this), thumbnail.getHeight(this), BufferedImage.TYPE_INT_ARGB);
